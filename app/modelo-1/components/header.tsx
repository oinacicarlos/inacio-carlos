  ...
  return (
    ...
    >
      <nav className="container mx-auto px-6 flex items-center justify-between md:px-[24]">
        <Link href="/modelo-1" className="flex items-center gap-2 group" onClick={scrollToTop}>
          <Image src="/modelo-1/images/hously-logo.svg" alt="Hously" width={120} height={32} className="w-auto h-6" />
        </Link>
        ...
        <ul className="hidden md:flex items-center gap-10 text-sm tracking-wide">
          {[
            ...
            <li key={item.label}>
              <Link
                href={item.href}
                className="hously-header-link hover:text-[rgb(251,146,60)] transition-colors duration-300 relative after:absolute after:bottom-0 after:left-0 after:h-px after:w-0 hover:after:w-full after:bg-[rgb(251,146,60)] after:transition-all after:duration-300 text-white"
              >
                {item.label}
              </Link>
            </li>
