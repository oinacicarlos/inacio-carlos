...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  const { id } = await context.params
  const { error } = await admin.supabase.from("whatsapp_conversations").delete().eq("id", id)

  if (error) {
    return NextResponse.json({ ok: false, error: "Não consegui excluir essa conversa." }, { status: 500 })
  }
